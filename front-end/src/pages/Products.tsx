...
const Products = () => {
  ...
  if (error) {
    return (
      <div className="container mx-auto px-2 sm:px-4 py-6 sm:py-8">
        <div className="text-center">
          <p className="text-red-600 mb-3 sm:mb-4">{t('products.error')}</p>
          <button 
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-6 sm:py-8">
      <div className="text-center mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-4xl font-bold mb-2 sm:mb-4">{t('products.title')}</h1>
        <p className="text-base sm:text-lg text-gray-600 dark:text-gray-300">{t('products.all')}</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 sm:mb-6 gap-2 sm:gap-0">
        <div className="flex items-center space-x-2 sm:space-x-4">
          <label className="text-xs sm:text-sm font-medium">{t('products.sortBy')}:</label>
          <select
            value={sortBy}
      ...
      {filteredAndSortedProducts.length === 0 ? (
        <div className="text-center py-8 sm:py-12">
          <FaBox className="text-4xl sm:text-6xl text-gray-300 mx-auto mb-2 sm:mb-4" />
          <p className="text-gray-600 dark:text-gray-300 text-base sm:text-lg">{t('products.noProducts')}</p>
        </div>
      ) : (
            ...
            >
              <div className="relative flex items-center justify-center bg-gray-100 dark:bg-gray-900 aspect-[4/3] w-full">
                <img
                  ...
                  alt={product.name}
                  className="max-h-32 sm:max-h-44 max-w-full object-contain transition-transform duration-300"
      ...
      {selectedProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-1 sm:p-4 z-50 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden shadow-2xl transform transition-all duration-500 scale-95 hover:scale-100 mx-2 sm:mx-4 animate-slideUp">
            {/* Header with gradient background */}
            <div className="relative bg-gradient-to-r from-primary to-primary/80 text-white p-4 sm:p-6 rounded-t-2xl">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl sm:text-3xl font-bold mb-1 sm:mb-2">{selectedProduct.name}</h2>
                  <div className="flex items-center text-white/90">
                    <FaShoppingCart className="mr-2" />
                    <span className="text-xs sm:text-sm">{t('product.premiumProduct')}</span>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedProduct(null)}
                  className="text-white/80 hover:text-white text-xl sm:text-2xl font-bold transition-colors duration-200 hover:scale-110"
                >
                  ✕
                </button>
              </div>
            ...
            {/* Content */}
            <div className="p-2 sm:p-4 overflow-y-auto max-h-[calc(90vh-120px)] sm:max-h-[calc(90vh-200px)]">
              {/* Image Section */}
              <div className="mb-3 sm:mb-6 relative group">
                <div className="overflow-hidden rounded-xl shadow-lg bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800">
                  <img
                    ...
                    className="w-full h-32 sm:h-48 object-contain transition-transform duration-300 group-hover:scale-105"
                    onError={(e) => {
                      const target = e.target as HTMLImageElement;
                      target.src = 'https://images.unsplash.com/photo-1607619056574-7b8d3ee536b2?w=400&h=300&fit=crop';
                    }}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 mb-3 sm:mb-6">
                <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 p-3 sm:p-4 rounded-xl border border-blue-200 dark:border-blue-700">
                  <div className="flex items-center mb-1 sm:mb-2">
                    <div className="bg-blue-500 text-white p-2 rounded-lg mr-2 sm:mr-3">
                      <span className="text-base sm:text-lg font-bold">{selectedProduct.category}</span>
                    </div>
                    <h3 className="font-semibold text-blue-800 dark:text-blue-200 text-xs sm:text-base">{t('product.category')}</h3>
                  </div>
                  <p className="text-blue-700 dark:text-blue-300 text-xs sm:text-sm">{selectedProduct.category}</p>
                </div>
                <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 p-3 sm:p-4 rounded-xl border border-green-200 dark:border-green-700">
                  <div className="flex items-center mb-1 sm:mb-2">
                    <div className="bg-green-500 text-white p-2 rounded-lg mr-2 sm:mr-3">
                      <span className="text-base sm:text-lg font-bold">{selectedProduct.inStock}</span>
                    </div>
                    <h3 className="font-semibold text-green-800 dark:text-green-200 text-xs sm:text-base">{t('product.stock')}</h3>
                  </div>
                  <p className="text-green-700 dark:text-green-300 text-xs sm:text-sm">{selectedProduct.inStock > 0 ? `${selectedProduct.inStock} ${t('product.inStock')}` : t('product.outOfStock')}</p>
                </div>
              </div>
              {/* Description */}
              <div className="mb-3 sm:mb-6 p-2 sm:p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                <h3 className="font-semibold text-gray-800 dark:text-white mb-1 sm:mb-2 text-xs sm:text-base">{t('product.description')}</h3>
                <p className="text-gray-600 dark:text-gray-300 text-xs sm:text-sm leading-relaxed line-clamp-3 sm:line-clamp-none">{selectedProduct.description}</p>
              </div>
              {/* Stores */}
              ...
              <div className="flex flex-col gap-2 sm:gap-3 mt-2 sm:mt-4">
                <div className="flex-1 bg-gradient-to-r from-primary/10 to-primary/5 p-3 sm:p-4 rounded-xl border border-primary/20">
                  <div className="text-center">
                    <div className="text-lg sm:text-2xl font-bold text-primary mb-1">{formatPrice(selectedProduct.price, language)}</div>
              ...
              <div className="mt-3 sm:mt-6 p-2 sm:p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                <div className="flex items-center justify-center text-gray-600 dark:text-gray-300 text-xs sm:text-sm">
